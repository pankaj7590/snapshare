 ...
 * @modification Pankaj Salokhe <pankaj@fierydevs.com>
 */
 ...
 * @package demi\backup
 */
class BackupComponent extends \yii\base\Component
{
    /** @var boolean If gzip compression is to be used for final output. By default false. e.g. true or false */
    public $useFileGZipCompression = false;
	
    /** @var boolean If gzip compression is to be used for database export file. By default false. e.g. true or false */
    public $useDbGZipCompression = false;
     ...
     * if callable:
     * function(\common\components\BackupComponent $component) {
    ...
    public $directories = [];
    /**
     * Host name or IP address. By default 'localhost'.
     * If you want to set it to different host name then set it. eg. '123.456.789.000'
     *
     * @var string
     */
     ...
     * @var string
     */
    public $mysqldump = 'mysqldump --add-drop-table --allow-keywords -q -c -u "{username}" -h "{host}" -p\'{password}\' {db}';
    ...
    public function backupDatabase($saveTo)
    {
        ...
        foreach ($this->databases as $name => $params) {
			...
			//MODIFICATION BY PANKAJ
			if($this->useFileGZipCompression){
				$command = isset($params['command']) && !empty($params['command']) ? $params['command'] : $this->mysqldump.' | gzip -9';
			}else{
    ...
    public function deleteJunk()
    {
        ...
        foreach ($files as $file) {
            if (@unlink($file)) {
                Yii::info('Backup file was deleted: ' . $file, 'common\components\BackupComponent::deleteJunk()');
            } else {
                Yii::error('Cannot delete backup file: ' . $file, 'common\components\BackupComponent::deleteJunk()');
            }
        }
